...
void setup() {
  ...
  //cursor(soccerBall);  
  writeSimulatorName(); 
  port = new Serial(this, "COM4", 9600); //initializing here fixed one of the ports always being null
  port2 = new Serial(this, "COM5", 19200);
  setUpSerialPort();
...
void mousePressed()
{
  ...
  {
    //used to set up the room
    if(!masterPadSecondClick && (connection || connection2))
    {
      setUpRoom(p);    
    }
...
void setUpSerialPort()
{
  //String portName = "";
    ...
    for(int i = 0; i < 2; i++) //not commented
    { //not commented
      //portName = "COM4"; //

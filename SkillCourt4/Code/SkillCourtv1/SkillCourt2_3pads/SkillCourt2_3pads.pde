...
void setup() {
  ...
  //cursor(soccerBall);  
  writeSimulatorName(); 
  port = new Serial(this, "COM3", 9600); //initializing here fixed one of the ports always being null
  ...
  if(masterPadSecondClick)
  {
    if(pl.playing())
    {
      if(pl.hasStarted())
      {
        ...
        else if(voltage >= 6 && voltage < 11)
        {
          println("voltage = " + voltage);
          double massOfBall = 0.45;//in grams
          //println("Calling Play.hit()-----------------");
          step s = pl.hit(335,35,millis(), millis()-1, massOfBall);
            
          //println("x = " + mouseX);
          //println("y = " + mouseY);
          handleStep(s);
        }
        else if(voltage >= 11 && voltage < 16)
        {
          println("voltage = " + voltage);
          double massOfBall = 0.45;//in grams
          //println("Calling Play.hit()-----------------");
          step s = pl.hit(260,110,millis(), millis()-1, massOfBall);
    ...
    {
      tryToStablishConnectionTest(); //tryToStablishConnectionTest();
    }
  }
  draw4Lines();
}
...
void serialEvent(Serial p) { 
  if (p == port)
  {
  println("Serial event detected port1");
  String inString =  p.readString();  //p.readString()
  if(!connection)
  {
    ...
    else if(inString.equals("E"))
    {
      ...
      if(!pl.playing())
      {
        ...
        if(!masterPadSecondClick)
        {
          ...
          //pad pb = skillCourt.get_pad(0, 0, 1);
          pa.set_master(true);
          //pb.set_master(true);
        }
        pl = new play(command, skillCourt);
...
if (p == port2)
  {
  println("Serial event detected port2");
  String inString2 =  p.readString(); //p.readString()
  if(!connection2)
  {
    if(inString2.equals("B"))
    {
      port2 = p;
      connection2 = true;
      port2.clear();
  ...
  {
    println("Problem1");
    if(inString2.equals("S"))
        command = "";
    else if(inString2.equals("z"))
      command = command + "0";
    else if(inString2.equals("E"))
    {
      pl = new play(command, skillCourt);
      startRoutineTime = millis();
      if(!pl.playing())
      ...
      command = command + inString2;
    
    println("Problem2");
  }
  else if(pl.playing())
  {
    voltage = Integer.parseInt(inString2);
    voltage += 5;
  }
  println(inString2); 
} 
...
if (p == port3)
  {
  println("Serial event detected port3");
  String inString3 =  p.readString(); //p.readString()
  if(!connection3)
  {
    if(inString3.equals("A"))
    {
      port3 = p;
      connection3 = true;
      port3.clear();
  ...
  {
    println("Problem1");
    if(inString3.equals("S"))
        command = "";
    else if(inString3.equals("z"))
      command = command + "0";
    else if(inString3.equals("E"))
    {
      pl = new play(command, skillCourt);
      startRoutineTime = millis();
      if(!pl.playing())
      ...
      command = command + inString3;
    
    println("Problem2");
  }
  else if(pl.playing())
  {
    voltage = Integer.parseInt(inString3);
    voltage += 10;
  }
  println(inString3); 
}
...
void mousePressed()
{
  ...
  {
    //used to set up the room
    if(!masterPadSecondClick && (connection || connection2 || connection3))
    {
      setUpRoom(p);    
    }
...
void setUpSerialPort()
{
  //String portName = "";
    ...
    for(int i = 0; i < 3; i++) //not commented
    { //not commented
      //portName = "COM4"; //
    ...
    if (i == 2)
      ...
      try
      {
        //port3 = new Serial(this, "COM8", 38400);
        //port3.bufferUntil(' ');
        //need to do a checking send and receive info to make sure it is the device I am looking for
        //println(portName);
        int c = port3.read();
  ...
  if(port == null || port2 == null || port3 == null)
  {
    fill(0, 0, 0);
    textSize(20);
...
void changePadColor(pad p, colour c)
{
  ...
  if(true) //was if(p.is_master())
  {
    println("MASTERRRRRRRRR");
    
    if(p == skillCourt.get_pad(0, 0, 0))
    {
    ...
    else if(p == skillCourt.get_pad(0, 0, 1))
    {
      if(c.getRed()>0 && c.getGreen()>0 && c.getBlue()>0) //c.getRed()>0 && c.getGreen()>0 && c.getBlue()>0
    {
      println("OFF");
    ...
    else if(p == skillCourt.get_pad(0, 1, 0))
    {
      if(c.getRed()>0 && c.getGreen()>0 && c.getBlue()>0) //c.getRed()>0 && c.getGreen()>0 && c.getBlue()>0
    {
      println("OFF");
...
void tryToStablishConnection()
{
  ...
  {
    String str2 = port2.readStringUntil('o');
    if(str2 == null)
      return;
  ...
  {
    String str3 = port3.readStringUntil('o');
    if(str3 == null)
      return;
